...
def _run(rank, world_size, cfg):
    ...
    # === Finetuning Modification: Load Pretrained Weights ===
    if hasattr(cfg, 'pretrained_model_path') and cfg.pretrained_model_path:
        ...
        try:
            # Try loading HuggingFace weights
            pretrained_model, _, _ = load_model_hf(cfg.pretrained_model_path, device, local_files_only=True)
            # Load weights into current model
            score_model.load_state_dict(pretrained_model.state_dict())
            del pretrained_model
            torch.cuda.empty_cache()
            mprint("Successfully loaded pretrained weights!")
        except Exception as e:
            mprint(f"Failed to load pretrained weights: {e}")
            raise e
    else:
        mprint("No pretrained model path provided. Training from scratch (Warning: This is run_finetune.py).")
    # ========================================================
            ...
            if step > 0 and step % cfg.training.snapshot_freq == 0 or step == num_train_steps:
                ...
                if cfg.training.snapshot_sampling:
                    ...
                    if cfg.eval.perplexity:
                        with torch.no_grad():
                            eval_model = GPT2LMHeadModel.from_pretrained("gpt2-large", local_files_only=True).to(device).eval()
